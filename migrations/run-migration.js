...
const seq = new Sequelize(process.env.DB_NAME || 'chattingan', process.env.DB_USER || 'root', process.env.DB_PASSWORD || '', {
    ...
    for (const q of queries) {
      try {
        ...
        if (e.message.includes('Duplicate column') || e.message.includes('already exists')) {
          console.log('SKIP: ' + q.split(' ').slice(2, 4).join(' ') + ' (already exists)');
        } else {
